...
# Execute queries and get dataframes
    df_raw_sump = execute_query_and_return_df("sqlTelemetry", query_formatted_raw_sump)
    df_raw_flow_meter = execute_query_and_return_df("sqlTelemetry", query_formatted_raw_flow_meter)
    df_rainfall = execute_query_and_return_df("DALMeteorology", query_formatted_rainfall)
    ...
    if df_raw_sump is not None:
        ...
        #df_raw_sump['TimeGMT'] = pd.to_datetime(df_raw_sump['TimeGMT']).dt.tz_localize(None)
        # Save to xlsx

...
print('DB_Addr_sump:', DB_Addr_sump.tolist())
print('Source_sump:', Source_sump.tolist())

# Remove the first character from each element in the Series and join them into a single string
DB_Addr_sump_str = ''.join(DB_Addr_sump.apply(lambda x: x[1:] if isinstance(x, str) else x))

# Convert the 'Source' Series to a single string
Source_sump_str = ''.join(Source_sump.apply(lambda x: x if isinstance(x, str) else x)).upper()

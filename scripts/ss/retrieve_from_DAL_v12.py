...
for index, row in date_ranges_df.iterrows():
    ...
    if df_rainfall is not None:
        ...
        df_rainfall.to_excel(f'../data/raw/site{site_id}_from_{start_date}_to_{end_date}_rainfall.xlsx', index=False)
    
    if df_raw_sump is not None:
        print("Head of df_raw_sump:")
        print(df_raw_sump.head(5))
        df_raw_sump.to_excel(f'../data/raw/site{site_id}_from_{start_date}_to_{end_date}_raw_sump.xlsx', index=False)
    
    if df_hour_agg_flow_meter is not None:
        print("Head of df_hour_agg_flow_meter:")
        print(df_hour_agg_flow_meter.head(5))
        df_hour_agg_flow_meter.to_excel(f'../data/raw/site{site_id}_from_{start_date}_to_{end_date}_hour_agg_flow_meter.xlsx', index=False)
    
    '''
    if df_daily_agg_sump is not None:
        print("Head of df_daily_agg_sump:")
        print(df_daily_agg_sump.head(5))
        df_daily_agg_sump.to_excel(f'../data/raw/site{site_id}_from_{start_date}_to_{end_date}_daily_agg_sump.xlsx', index=False)
    
    if df_raw_flow_meter is not None:
        print("Head of df_raw_flow_meter:")
        print(df_raw_flow_meter.head(5))
        df_raw_flow_meter.to_excel(f'../data/raw/site{site_id}_from_{start_date}_to_{end_date}_raw_flow_meter.xlsx', index=False)
    '''